...
Future deleteDepartement(int id){
  return http.delete(Uri.parse("http://10.0.2.2:8081/dep/delete?id=${id}"));
}
...
Future addDepartement(Departement dep)async
{
  ...
  Response response = await http.post(Uri.parse("http://10.0.2.2:8081/dep/add"),
      headers:{
...
Future updateDepartement(Departement dep)async
{
  ...
  Response response = await http.put(Uri.parse("http://10.0.2.2:8081/dep/update"),
      headers:{
        "Content-type":"Application/json"
      },body:jsonEncode(<String,dynamic>{
        "idDep":dep.idDep,
...
Future<List<Departement>> getClassesByDepartementQuery(int id) async {
  Response response = await http.get(
    Uri.parse("http://10.0.2.2:8081/dep/getByIdDepartement"),
  );
