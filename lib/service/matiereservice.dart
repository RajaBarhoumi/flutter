...
Future<List<Matiere>> getAllMatieres() async {
  try {
    Response response = await http.get(Uri.parse("http://10.0.2.2:8081/matiere/all"));
...
Future addMatiere(Matiere matiere)async
{
  ...
  Response response = await http.post(Uri.parse("http://10.0.2.2:8081/matiere/add"),
      headers:{
...
Future updateMatiere(Matiere matiere)async
{
  ...
  Response response = await http.put(Uri.parse("http://10.0.2.2:8081/matiere/update"),
      headers:{
        "Content-type":"Application/json"
      },body:jsonEncode(<String,dynamic>{
